...
import { apiGetProducts, apiGetCategoryName} from "../../../api";
import Product from "../../../ui/public/Product";
import { useParams, useRouter, useSearchParams } from "next/navigation";
import { SearchItem, InputSelect, Breadcrumb, Pagination } from "../../../components/Index";
import { sorts } from "../../../util/contant";
import Image from "../../../../public/img/banner/HS-THB-1300k-TGHS138-web-pre.jpg"
const Page = () => {
  ...
  const fetchCategory = async () => {
    const response = await apiGetCategoryName(slug);
    if(response.success) setCategoryName(response?.data?.name);
  }
  console.log(categoryName)
  const fetchProducts = async (queries) => {
    const response = await apiGetProducts({ ...queries, page, limit: 8, category: categoryName });
  ...
  useEffect(() => {
    ...
    router.push(`/category/${slug}?${searchString}`);
  }, [searchParams, sort]);
 
  return (
    <div>
      <Breadcrumb category='Category' name={categoryName}/>
