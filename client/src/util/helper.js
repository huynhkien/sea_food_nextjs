...
export const validate = (payload, setInvalidFields) => {
  ...
  Object.entries(payload).forEach(([fieldName, fieldValue]) => {
    if (typeof fieldValue === 'string' && fieldValue.trim() === '') {
      newInvalidFields.push({ name: fieldName, message: 'Require this field' });
      invalids++;
    } else {
      switch (fieldName) {
        case 'email':
          ...
          if (!fieldValue.match(emailRegex)) {
            newInvalidFields.push({ name: fieldName, message: 'Email invalid' });
            invalids++;
          }
          break;
        case 'password':
          if(fieldValue.length < 6){
            newInvalidFields.push({ name: fieldName, message: 'Password minimum 6 characters' });
            invalids++;
          }
